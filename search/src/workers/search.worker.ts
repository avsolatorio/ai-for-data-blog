...
// Load ONNX WASM from CDN rather than inlining it in the worker bundle.
// Without this, Rollup base64-encodes the .wasm files, inflating the worker
// from ~3 MB to ~63 MB. The version must match onnxruntime-web in package.json.
// @ts-ignore — ort property may not be in the type definitions
...
function enrichFromTitlesMap(results: SearchResult[]): SearchResult[] {
  ...
  return results.map(r => {
    ...
    return { ...meta, ...r, title: (meta.title as string) || r.title } as SearchResult
  })
}
...
async function loadModel(modelId?: string): Promise<void> {
  postMsg({ type: 'progress', phase: 'model', message: 'Loading embedding model…' })
  try {
  ...
  postMsg({ type: 'progress', phase: 'model', message: 'Embedding model ready' })
}
...
async function getEmbedding(text: string): Promise<Float32Array> {
  ...
  // GIST-small-Embedding-v0: CLS token pooling + Normalize().
  // fp32 dtype required — q8 loads a different quantized file that produces
  // different vectors from what the index was built with.
...
async function initIndex(manifestUrl: string): Promise<void> {
  postMsg({ type: 'progress', phase: 'index', message: 'Fetching index manifest…' })

  const resp = await fetch(manifestUrl)
  if (!resp.ok) {
    throw new Error(`Failed to fetch manifest: ${manifestUrl} (HTTP ${resp.status})`)
  }
  ...
  if (manifest.search_mode === 'flat') {
    postMsg({ type: 'progress', phase: 'index', message: 'Loading flat index…' })
    ...
    // HNSW mode
    postMsg({ type: 'progress', phase: 'index', message: 'Loading HNSW index…' })
    const engine = new HNSWEngine()
    const titlesUrl = manifest.index?.titles ? baseUrl + manifest.index.titles : null
    const bm25Url = manifest.index?.bm25_corpus ? baseUrl + manifest.index.bm25_corpus : null

    const [, titlesData, bm25Data] = await Promise.all([
      engine.init(baseUrl, { cacheName: CACHE_NAME, manifest }),
      titlesUrl
        ? fetchJson<Record<string, Record<string, unknown>>>(titlesUrl).catch(() => null)
        : Promise.resolve(null),
      bm25Url
        ? fetchJson<BM25CorpusEntry[]>(bm25Url, { cacheName: CACHE_NAME }).catch(() => null)
        : Promise.resolve(null),
    ])
    searchEngine = engine
    ...
    try {
      ...
      const idToMeta = (id: number | string): Partial<SearchResult> => {
        ...
        if (titlesMap) {
          ...
          return meta
            ? ({ ...meta, title: item.title, text: item.text } as Partial<SearchResult>)
...
async function init(manifestUrl: string, modelId?: string): Promise<void> {
  try {
    ...
    isModelReady = true
    postMsg({ type: 'ready', mode: manifest?.search_mode ?? 'flat', config: manifest! })
  } catch (err) {
...
self.onmessage = async (e: MessageEvent<WorkerInboundMessage | { ping?: boolean; text?: string }>) => {
  const msg = e.data
  ...
  if (!('type' in msg)) {
    ...
    if ((msg as any).text && isIndexReady) {
      ...
      try {
        ...
        if (hybridEngine && vec) {
          const results = await hybridEngine.search(vec, text, { topK: 20, mode: 'hybrid' })
          postMsg({ type: 'results', data: enrichFromTitlesMap(results), fallback: false })
        } else if (hybridEngine && !vec) {
          const results = await hybridEngine.search(null, text, { topK: 20, mode: 'lexical' })
          postMsg({ type: 'results', data: enrichFromTitlesMap(results), fallback: true })
        } else if (engine && vec) {
          const results = await (engine as SearchEngine).search(vec, { topK: 20 })
          postMsg({ type: 'results', data: enrichFromTitlesMap(results as SearchResult[]), fallback: false })
        }
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
        postMsg({ type: 'error', message })
  ...
  switch ((msg as WorkerInboundMessage).type) {
    ...
    case 'search': {
      ...
      const { text, topK = 20, ef = 50, ef_upper = 2, threshold = 0.0, mode = 'hybrid' } = m
      const useFallback = !isModelReady && (mode === 'semantic' || mode === 'hybrid')
      ...
      try {
        ...
        if (mode === 'lexical' || useFallback) {
          ...
          if (hybridEngine) {
            results = await hybridEngine.search(null, text, { topK, ef, mode: 'lexical' })
          } else {
            results = []
          }
          postMsg({
        ...
        if (mode === 'semantic') {
          if (hybridEngine) {
            results = await hybridEngine.search(vec, text, { topK, ef, mode: 'semantic' })
          } else if (searchEngine) {
            const raw = await searchEngine.search(vec, { topK, ef, ef_upper, threshold })
          ...
          // hybrid
          if (hybridEngine) {
            results = await hybridEngine.search(vec, text, { topK, ef, mode: 'hybrid' })
          } else if (searchEngine) {
            const raw = await searchEngine.search(vec, { topK, ef, ef_upper, threshold })
            results = raw as SearchResult[]
          } else {
    ...
    case 'getRecent': {
      ...
      if (titlesMap) {
        ...
        recent = bm25Corpus.slice(0, limit).map(item => ({
          id: item.id,
          score: 0,
          title: item.title,
          text: item.text,
    ...
    case 'searchCompare': {
      ...
      try {
        ...
        const hnswResults = searchEngine
          ? ((await searchEngine.search(vec, { topK, ef, ef_upper })) as SearchResult[])
          : []
