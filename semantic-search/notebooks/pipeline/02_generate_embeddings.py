...
def quantize_sq8(vec: np.ndarray) -> dict:
    """Per-vector symmetric int8 (SQ8): scale by max absolute value.

    Compression: float32 (4 bytes/dim) → int8 (1 byte/dim) = 4× reduction.
    Quality loss: cosine similarity error < 0.001 (validated in __main__).
    """
...
def dequantize_sq8(scale: float, qv: list) -> np.ndarray:
